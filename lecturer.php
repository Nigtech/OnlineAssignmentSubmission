...
if (isset($_POST['submit'])) {
    ...
    if (empty($throwError)) {
        ...
        $checkEmailStmt = "SELECT * FROM lecturer WHERE email = '$email'";
        ...
        $checkUsernameStmt = "SELECT * FROM lecturer WHERE username = '$username'";
        ...
        $checkRegNumberStmt = "SELECT * FROM lecturer WHERE IDNumber = '$idNumber' ";
        ...
        $stmt = $conn->prepare("INSERT INTO lecturer(`firstName`, `lastName`, `IDNumber`, `faculty`, `username`, `email`, `password`) values ('{$fname}', '$lname', '{$idNumber}', '{$faculty}', '{$username}', '{$email}', '{$hashedPassword}')");
        if ($stmt->execute()) {
...
<body id="studentPortal">
    ...
    <form action="" method="post" id="studentRegistrationForm">
        ...
        <?php if (isset($throwError['idNumber'])): ?>
                <p style="color:red; margin-left: 40px; font-weight:bold;"><?=$throwError['idNumber']?></p>
        <?php endif ?>
        <input type="text" <?php if(isset($_POST['idNumber'])){echo "value='".$_POST['idNumber']."'";}?> name="idNumber" placeholder="enter your ID Number">
